...
#formula cost function with regularization
% regularize term - skipping theta(0)
% "end" keyword here refers to last element in matrix so we can subset the matrix
normalCost = 1/m * sum((-y.*log(A)) - ((1-y).*log(1-A)));
regCost = (lambda/(2*m)) * sum(theta([2:end]).^2);
J = normalCost+regCost;
#####gradeint descent### 
reg = (lambda/m) .*theta;
#skipping theta(0)
reg(1) = 0;
normalGrad = ((1 / m).* X' * (A - y)); #for each number in the matrix x'*A-y divide 1/m
#added the reg derivative
grad = normalGrad + reg;
